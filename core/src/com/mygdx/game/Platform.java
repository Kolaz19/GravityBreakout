...
public class Platform {
    ...
    private final float box2DPosY = 8 / MainGame.PIXELS_TO_METERS;
    ...
    public Platform(World world, int backgroundWidth) {
        ...
        bodyDef.position.x = (backgroundWidth / 2) / MainGame.PIXELS_TO_METERS;
        bodyDef.position.y = box2DPosY;
        //Polugon Shape
        PolygonShape shape = new PolygonShape();
        shape.setAsBox(width / 2f / MainGame.PIXELS_TO_METERS, height / 2f / MainGame.PIXELS_TO_METERS);
        //Fixture
    ...
    private void updateCoordinate() {
        if (isMouseOverLeftBoundary()) {
            body.setTransform(boundaryWidth / MainGame.PIXELS_TO_METERS,box2DPosY,0);
        } else if (isMouseOverRightBoundary()) {
            body.setTransform((backgroundWidth - boundaryWidth) / MainGame.PIXELS_TO_METERS, box2DPosY, 0);
        } else {
    ...
    public Area getArea(float x) {
        ...
        float startPosWorld = this.body.getPosition().x * MainGame.PIXELS_TO_METERS - this.width / 2;
        ...
        if (x < body.getPosition().x * MainGame.PIXELS_TO_METERS) {
